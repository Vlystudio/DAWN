...
const EVAL: Record<string, Evaluator> = {
  ...
  design_system: () => ({ status: 'PARTIAL', works: ['Shell library incl. LAYOUT-SAFE VARIANTS: PageShell, PageShellSplit (master–detail), PageShellPanel, PageShellLog (fixed header + scroll box), PageShellCanvas — layout invariants unit-tested', 'Central status map is the single source for labels/tones (StatusBadge, System Health, Setup Center, Model Cookbook, Skills risk colours)', 'Migrated: System Health, Setup Center, Workspace, Email wizard, Model Cookbook, Logs (PageShellLog), Model Manager + Model Hub (PageShellPanel), Notes (PageShellSplit — proof pattern)'], missing: ['Still on bespoke layouts: Dashboard, Model Optimizer, Research, Documents, Tasks, Calendar, Tools/Skills, Security/Vault, Backup, integrations, Settings — migrate one at a time using the matching shell variant + a visual check'], nextAction: 'Roll out PageShellSplit to more master–detail screens after visually verifying Notes' }),
