...
const EVAL: Record<string, Evaluator> = {
  ...
  design_system: () => ({ status: 'PARTIAL', works: ['Shell library incl. LAYOUT-SAFE VARIANTS: PageShell, PageShellSplit (master–detail), PageShellPanel, PageShellLog (fixed header + scroll box), PageShellCanvas — layout invariants unit-tested', 'Central status map is the single source for labels/tones (StatusBadge, System Health, Setup Center, Model Cookbook, Skills risk colours)', 'Migrated so far: System Health, Setup Center, Workspace, Email wizard, Model Cookbook, Logs (PageShellLog)'], missing: ['Most legacy screens still on bespoke layouts: Dashboard, Model Hub/Manager/Optimizer, Research, Docs/Notes/Tasks/Calendar, Tools/Skills, Security/Vault, Backup, integrations, Settings — migrate one at a time using the matching shell variant + visual check'], nextAction: 'Migrate the next simple/panel screens using PageShellPanel; split screens using PageShellSplit' }),
