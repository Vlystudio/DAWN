...
const EVAL: Record<string, Evaluator> = {
  ...
  workspace_livehooks: () => ({ status: 'PARTIAL', works: ['Notes + Tasks register/update/prune instantly on CRUD (idempotent)', 'Reconcile remains the fallback (Workspace open + Brain rebuild)'], missing: ['Documents/Memories/Research/Benchmarks/Email/Knowledge are reconcile-only (no live hook yet)'], nextAction: 'Extend live hooks to more feature services' }),
