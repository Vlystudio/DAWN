...
const EVAL: Record<string, Evaluator> = {
  ...
  design_system: () => ({ status: 'PARTIAL', works: ['Shared library: PageShell, StatusBadge, HealthBadge, LoadingState, ErrorState, ActionBar, Button, DataTable', 'Central status map adopted (System Health, Setup Center, Model Cookbook)', 'Adopted in newer screens (System Health, Setup Center, Workspace, Email wizard, Model Cookbook)'], missing: ['Legacy screens (Dashboard, Model Hub/Manager/Optimizer, Research, Docs/Notes/Tasks/Calendar, Tools/Skills, Security/Vault, Backup, integrations, Settings, Logs) not yet migrated to PageShell/DataTable'], nextAction: 'Migrate remaining legacy screens to the shared library' }),
