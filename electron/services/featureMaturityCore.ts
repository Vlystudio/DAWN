...
const EVAL: Record<string, Evaluator> = {
  ...
  workspace_livehooks: () => ({ status: 'PARTIAL', works: ['Notes, Tasks, Documents, Memories register/update/prune instantly on CRUD (idempotent)', 'Reconcile remains the fallback (Workspace open + Brain rebuild)'], missing: ['Research/Benchmarks/Email/Knowledge remain reconcile-only (no live hook yet)'], nextAction: 'Extend live hooks to research/benchmarks/email/knowledge' }),
  ...
  design_system: () => ({ status: 'PARTIAL', works: ['Shared library: PageShell, StatusBadge, HealthBadge, LoadingState, ErrorState, ActionBar, Button, DataTable', 'Central status map is the single source for labels/tones — adopted by StatusBadge, System Health, Setup Center, Model Cookbook, and Skills risk colours', 'PageShell/DataTable used by the newer screens (System Health, Setup Center, Workspace, Email wizard, Model Cookbook)'], missing: ['Legacy screen LAYOUTS (Dashboard, Model Hub/Manager/Optimizer, Research, Docs/Notes/Tasks/Calendar, Tools/Skills, Security/Vault, Backup, integrations, Settings, Logs) not yet on PageShell/DataTable — they have split/flex-scroll layouts that need careful, visually-verified migration'], nextAction: 'Migrate legacy screen layouts to PageShell/DataTable, one at a time, with visual verification' }),
