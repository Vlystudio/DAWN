...
function rec(provider: HelperProvider, ok: boolean, queueStatus?: string, reason?: string) {
  try { analytics.record({ role: 'entailment_verifier', provider: provider as any, status: statusFor(provider as any, ok, queueStatus), reason, generation: helperQueue.generation }); } catch { /* analytics must never break verification */ }
}
...
export async function verifyClaim(claim: string, evidence: string): Promise<EntailOut> {
  ...
  rec(res.provider, r.ok, (r as any).status, (r as any).reason);
