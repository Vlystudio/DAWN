...
export interface Settings {
  ...
  rerankerEnabled: boolean;           // master toggle for embedding/heuristic rerank stage (default off)
  rerankerModelPath: string;
  maxRerankCandidates: number;
  rerankTimeoutMs: number;
  // Reranker provider selection + dedicated GGUF cross-encoder runtime (a real /rerank llama-server).
  //   provider: which reranker runs when reranking is active. Default embedding_similarity (ready, local).
  //   gguf.*: a SECOND llama-server started with --reranking that serves a real GGUF reranker model.
  reranker: {
    provider: 'disabled' | 'embedding_similarity' | 'heuristic' | 'gguf_reranker';
    gguf: {
...
export const DEFAULTS: Settings = {
  ...
  reranker: {
    provider: 'embedding_similarity',
    gguf: { enabled: false, modelPath: '', port: 8091, contextSize: 4096, threads: 0, gpuLayers: 0, batchSize: 0, startupTimeoutMs: 60000, requestTimeoutMs: 10000, autoStart: false, keepWarm: false, idleStopMs: 300000, topKInput: 30, topKOutput: 8, maxCandidateChars: 4000, queueCapacity: 16, maxConcurrency: 1 },
  },
  helperModels: { queryRewriteModel: '', hydeModel: '', entailmentModel: '', rerankerModel: '', preferChatModelFallback: true, timeoutMs: 8000, maxTokens: 200, adaptiveRouting: { enabled: false, minSamples: 12, slowP95Ms: 3500, timeoutRateThreshold: 0.20, failureRateThreshold: 0.30, cooldownMs: 300000, recoveryMinSamples: 8, recoveryP95Ms: 2000, recoveryTimeoutRate: 0.10, applyToRewrite: true, applyToHyDE: true, applyToEntailment: true, applyToReranker: true, persistSafeHistory: false } },
